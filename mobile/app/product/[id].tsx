...
const ProductDetailScreen = () => {
  ...
  const handleOffer = () => {
    ...
    if (!Number.isFinite(price) || price <= 0) {
      Alert.alert("Enter an offer", "Enter a valid price for your offer.");
      return;
    }
    createOffer.mutate(
      { productId: product!._id, offeredPrice: price, quantity, message: offerMessage },
      {
        onSuccess: () => {
        ...
        onError: (error: any) => Alert.alert("Offer failed", error?.response?.data?.message || "Could not send your offer."),
      }
    );
  };
